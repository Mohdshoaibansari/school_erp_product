...
### 2.4 Deferred pending backend routes (resolved 2026-08-16)

These are UI-deferred because the backend does not yet expose the required routes (see ADR R6, R9–R11):
- **Subjects & subject groups** (C-05): read-only listing; create/edit/assign deferred.
- **Institution-type deactivate** (C-01): list/create/edit only; deactivate deferred.
- **Fee-assignment remove** (Fees): create/edit/waive only; remove deferred (no DELETE endpoint).
- **Cohort bulk fee assignment** (Fees): per-student now; cohort bulk flagged behind `fees.cohortBulkAssignment` pending the R6 backend change.
...
| P1-AC-14 | Platform Owner can manage institution types (list/create/edit; deactivate deferred) via the Institution Types screen. |
...
| P2-AC-4 | Subjects and subject groups can be listed read-only via the subjects/subject-groups endpoints (create/edit/assign deferred). |
...
| P3-AC-2 | Institution Admin can create, edit, and waive fee assignments (remove deferred — no backend DELETE endpoint). |
