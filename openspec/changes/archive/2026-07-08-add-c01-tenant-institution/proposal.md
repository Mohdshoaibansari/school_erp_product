...
- **Tech stack:** RESOLVED — locked by `docs/architecture/adr-platform-tech-stack.md` (Postgres+Supabase, Python+FastAPI, SQLAlchemy 2.0+Alembic, Supabase Auth JWT, Casbin RBAC+ABAC, pytest) and the platform software-architecture ADR `docs/architecture/adr-platform-software-architecture.md` (A1–A11: modular monolith, module manifest, monorepo, single Alembic env). These two ADRs are decisional input to the design/tasks artifacts.