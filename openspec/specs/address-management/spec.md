...
C-13 Address Management provides a single authoritative framework for addresses across the School ERP platform. It owns EntityType, AddressType, AddressTypeEntityType (compatibility matrix), Address, AddressAssignment (polymorphic entity + validity period), and geographic reference data (Country, State, City). All modules (Institution, Person, future Student/Employee) MUST consume C-13 instead of creating parallel address models. This spec is the behavioral source of truth derived from `docs/prd/C-13-Address-Management-PRD.md` and `docs/prd/c-13-impact-classification.md` via change `add-c13-address-management`.
